...
bool SimpleServer::init()
{
    ...
    if(set_options(udp_fd) == false)
    {
        LogErr("upd server fd set_options failed");
        unit();
        return false;
    }
...
bool SimpleServer::epoll_add_sock(int epoll_fd, int sock_fd)
{
...
bool SimpleServer::set_options(int fd)
{
...
void SimpleServer::epoll_del_sock(int epoll_fd, int sock_fd)
{
    struct epoll_event ee = { 0, { 0 } };
